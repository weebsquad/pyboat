...
import {compress, decompress} from './wasm/zlib.js';
...
export async function encode(data, {width, height, channels, depth = 8, level = 0}) {
    let offset = 0;
    let tmp_offset = 0;
...
export async function decode(array) {
    ...
    if (channels !== 4) {
        ...
        if (channels === 1) {
            ...
            } else if (channels === 3) {newPixels.fill(0xff);
        for (let i = 0; i < width * height; i++)
