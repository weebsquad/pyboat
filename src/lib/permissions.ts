...
export async function reportBlockedAction(member: discord.GuildMember, action: string) {
  ...
  if (isGlobalBlacklisted(member.user.id)) {
    ...
    await logDebug('BLACKLISTED_USER_ACTION', new Map([['_USERTAG_', getMemberTag(member)], ['_USER_ID_', member.user.id], ['_ACTION_', action]]));
  }
  if (isBlacklisted(member, true)) {
    ...
    await logCustom('CORE', 'BLACKLISTED_USER_ACTION', new Map([['_USERTAG_', getMemberTag(member)], ['_USER_ID_', member.user.id], ['_ACTION_', action]]));
  }
}
