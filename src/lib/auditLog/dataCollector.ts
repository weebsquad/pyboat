...
export const auditLogDefinitions: {[key: string]: any} = {
  ...
  CHANNEL_PINS_UPDATE: {
    ...
    validate: function(dt: any[], log: discord.AuditLogEntry, store: any) {
      ...
      if (
        ...
        return false;
      if (store instanceof discord.AuditLogEntry) return false;
  ...
  GUILD_BAN_ADD: {
    ...
    validate: (dt: any, log: any) => {
      return dt[0].user.id === log.targetId;
    },
  ...
  GUILD_EMOJIS_UPDATE: {
    ...
    60: (dt: any, log: any) => {
      ...
      let oldE = dt[1].emojis.find(
        (ele: discord.Emoji) => ele.id === log.targetId
      );
      if (typeof oldE !== 'undefined' || !(newE instanceof discord.Emoji))
        return false;
      if (newE.id !== log.targetId) return false;
      for (var key in log.changes) {
        let val = log.changes[key];
    ...
    61: (dt: any, log: any) => {
      ...
      if (!(newE instanceof discord.Emoji) || !(oldE instanceof discord.Emoji))
        return false;
      if (newE.id !== oldE.id) return false;
      for (var key in log.changes) {
        let val = log.changes[key];
        if (typeof newE[key] !== 'undefined' && newE[key] !== val.newValue)
          return false;
    ...
    62: (dt: any, log: any) => {
      ...
      if (typeof newE !== 'undefined' || !(oldE instanceof discord.Emoji))
        return false;
      if (oldE.id !== log.targetId) return false;
      for (var key in log.changes) {
        let val = log.changes[key];
        if (typeof oldE[key] !== 'undefined' && oldE[key] !== val.oldValue)
          return false;
  ...
  GUILD_MEMBER_UPDATE: {
    ...
    25: (dt: any, log: any, store: any) => {
      let newM = dt[0].roles;
      let oldM = dt[1].roles;
      let adds = newM.filter((ele: string) => {
        let had = oldM.find((ele2: string) => ele2 === ele);
        if (typeof had === 'undefined') had = false;
        return !had;
      });
      let removes = oldM.filter((ele: string) => {
        let have = newM.find((ele2: string) => ele2 === ele);
        if (typeof have === 'undefined') have = false;
        return !have;
      });
      let verifs = 0;
      for (var key in log.changes) {
        if (key !== '$add' && key !== '$remove') continue;
        let obj = log.changes[key]['newValue'];
        let cc = adds;
  ...
  MESSAGE_DELETE: {
    beforeFetch: function(dt: any[]) {
      ...
      if (msg.type !== discord.Message.Type.DEFAULT) return false;
      return true;
    },
    guildId: function(dt: any[]) {
      return dt[0].guildId;
    ...
    72: (
      ...
      if (log.actionType !== discord.AuditLogEntry.ActionType.MESSAGE_DELETE)
        return false;
      if (dt[1] === null || typeof dt[1] === 'undefined') return false; // We're not gonna validate channelids, lol. no decent option
      if (
    ...
    22: (dt: any, log: discord.AuditLogEntry.MemberBanAdd) => {
      if (dt[1] === null || typeof dt[1] === 'undefined') return false;
      let author = dt[1].author.id;
      if (log.targetId === author) return true;
      return false;
    },
    store: {
      entryFound: true
  ...
  MESSAGE_DELETE_BULK: {
    ...
    validate: (
      ...
      if (dt[0].channelId !== log.targetId) return false;
      if (store instanceof discord.AuditLogEntry.MessageBulkDelete) {
        if (dt[0].ids.length.toString() !== log.options.count.toString()) {
          if (log.id === store.id) return true; // lol
          // If counts are different from logs, let's add to our stored value and validate
          let storedCount = +store.options.count;
          let gottenCount = +log.options.count;
          if (storedCount + dt[0].ids.length === gottenCount) return true; // stored + event gotten adds up to log entries
        }
        return false;
      } else {
        if (dt[0].ids.length.toString() === log.options.count.toString())
    ...
    returnData: (log: any, store: discord.AuditLogEntry.MessageBulkDelete) => {
      if (typeof store !== 'object') return log;
      let toDeduct = +log.options.count - +store.options.count;
  ...
  VOICE_STATE_UPDATE: {
    ...
    26: (
      ...
      if (voiceState.channelId === null || oldVoiceState.channelId === null)
        return false;
      if (!(store instanceof discord.AuditLogEntry.MemberMove)) return true;
      if (store.id === log.id && store.options.count !== log.options.count)
        return true;
      return false;
    },
    ...
    27: (
      ...
      if (
        ...
        return false;
      if (oldVoiceState.channelId === null) return false;
