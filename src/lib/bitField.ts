...
export class BitField {
  ...
  constructor(bits: string | number | BigInt) {
    if (typeof bits === 'string' && utils.isNumber(bits)) bits = BigInt(bits);
  ...
  add(...bits: any[]) {
    ...
    if (typeof this.bitfield === 'bigint') total = BigInt(total);
  ...
  *[Symbol.iterator]() {
  ...
  resolve(bit: number | BigInt | BitField = 0) {
    if (typeof bit === 'number' && bit >= 0) {
      if (typeof this.bitfield === 'bigint') bit = BigInt(bit);
      return bit;
    }
...
export class Permissions extends BitField {
  ...
  ALL: any = Object.values(this.FLAGS).reduce(function(all: any, p: any) {
    if (typeof p === 'bigint') all = BigInt(all);
    return all | p;
  }, 0);
