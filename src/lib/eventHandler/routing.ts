...
export function isModuleEnabled(modName: string) {
    if (typeof modName !== 'string' || modName === null) return false;
    let cfgMod = config['modules'][modName];
    if (typeof cfgMod !== 'object') return false;
    if (typeof cfgMod['enabled'] === 'boolean') return cfgMod['enabled'];
    if (typeof cfgMod['toggles'] === 'object' && typeof cfgMod['toggles']['enabled'] === 'boolean') return cfgMod['toggles']['enabled'];
    return false;
  }
