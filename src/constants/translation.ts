export class Language {
  ...
  export let countries = new Array<Country>(
    ...
    new Country(
      'GB',
      'United Kingdom',
      new Array(discord.decor.Emojis.FLAG_GB, discord.decor.Emojis.ENGLAND),
      ''
    ),
  ...
  let langs = JSON.parse(langJson);
  export let languages = new Array<Language>();
  langs.forEach(function(ll: any) {
    let engname = ll.English;
    if (engname.indexOf(';') > -1) engname = engname.split(';')[0];
    let newobj = new Language(ll.alpha2, engname);
  ...
  langtags.forEach(function(ll: any) {
    if (
      ...
      return; // lol
    let f = countries.findIndex(
      (e) => e.shortcode.toUpperCase() === ll['ISO3166-1-Alpha-2'].toUpperCase()
    );
    if (f === -1) return;
    let mainl = ll.Languages.split(',')[0].split('-')[0];
    if (mainl.length !== 2) return;
