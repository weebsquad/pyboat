...
export async function parseMessageContent(
  ...
  if (update) MAXLEN = Math.floor(MAXLEN / 2);
  ...
  let usrIds = new Map<string, string>();
  let channelIds = new Map<string, discord.GuildChannel>();
  ...
  let roles = cont.match(/<@&[0-9]{18}>/g)!.map(function(e) {
    e = e.split(' ').join('');
    e = e.substr(3).slice(0, -1);
    return e;
  });

  let channels = cont.match(/<#[0-9]{18}>/g)!.map(function(e) {
    e = e.split(' ').join('');
    e = e.substr(2).slice(0, -1);
    return e;
  }); /*
...
*/
  if (Array.isArray(channels)) {
    await Promise.all(
      channels.map(async function(u) {
        if (!channelIds.has(u) && u !== discord.getBotId()) {
          const chan = (await discord.getChannel(u)) as discord.GuildChannel;
          if (chan === null) return;
          channelIds.set(u, chan);
        }
      })
    );
    for (let [id, ch] of channelIds) {
      let ico = '#';
      if (ch.type === discord.Channel.Type.GUILD_VOICE) ico = '🔊';
      if (ch.type === discord.Channel.Type.GUILD_CATEGORY) ico = '‣';
  ...
  if (cont.length > 0) {
    if (cont.includes('\n') || cont.length >= LEN_CODEBLOCKS) {
      ...
      if (!cutVertical && cont.length > MAXLENSINGLELINE)
        cont =
          cont.substring(0, Math.min(cont.length, MAXLENSINGLELINE)) + ' [...]';
...
export function getLogMessage(
  ...
  if (auditLog === true) {
    let obj = config.messagesAuditLogs[eventName][eventAction] as string;
    if (
