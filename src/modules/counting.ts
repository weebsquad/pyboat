...
function isPinnable(num: number) {
  if (
    ...
    return;
  if (cfgMod.autoPins.single.indexOf(num) > -1) return true;
  let checkRep = cfgMod.autoPins.repeating.find((numbr) => num % numbr === 0);
  if (typeof checkRep === 'number') return true;
  let checkRepLast = cfgMod.autoPins.repeatingLast.find((numbr) => {
    var lastChars = num.toString().slice(-numbr.toString().length); //gets last character
    return lastChars === numbr.toString();
  });
  if (typeof checkRepLast === 'number') return true;
  return false;
}
...
export async function OnMessageCreate(
  ...
  if (cfgMod.channels.indexOf(message.channelId) === -1) return;
  if (
    (message.type !== discord.Message.Type.DEFAULT ||
      typeof message.webhookId === 'string') &&
    cfgMod.useWebhook
  ) {
    if (isPinnable(+message.content)) await message.setPinned(true);
    return false;
  }
  ...
  if (message.author.bot === true) return await delet(message);
  ...
  if (!isNormalInteger(cont)) return await delet(message);
  let num = 0;

  try {
    let testn = await kv.get(`${cfgMod.keyCount}${message.channelId}`);
    if (typeof testn === 'number') num = testn;
  } catch (e) {}
  if (cont !== (num + 1).toString()) return await delet(message);
  let lastUsr;
  try {
    let testid = await kv.get(`${cfgMod.keyLastUser}${message.channelId}`);
    if (typeof testid === 'string') lastUsr = testid;
  } catch (e) {}
...
async function MessageDeletedChecks(message: discord.GuildMemberMessage) {
  ...
  let lastId;
  try {
    let testn = await kv.get(`${cfgMod.keyLastMid}${message.channelId}`);
    if (typeof testn === 'string') lastId = testn;
  } catch (e) {}
  if (num < 1 || cont !== num.toString()) return;
  if (typeof lastId !== 'string' || lastId !== message.id) return;
  await kv.put(`${cfgMod.keyCount}${message.channelId}`, num - 1);
  await kv.delete(`${cfgMod.keyLastUser}${message.channelId}`);
  await kv.delete(`${cfgMod.keyLastMid}${message.channelId}`);
  updateTopic(message.channelId);
...
export async function OnMessageDelete(
  ...
  if (cfgMod.channels.indexOf(messageDelete.channelId) === -1) return;
  if (
...
export async function OnMessageUpdate(
  ...
  if (cfgMod.channels.indexOf(oldMessage.channelId) === -1 || cfgMod.useWebhook)
    return;
  if (message.author === null || message.member === null) return;
  ...
  let contNew = message.content;
  let contOld = oldMessage.content;
  if (isNormalInteger(contOld) && !isNormalInteger(contNew))
    return await delet(message);
  if (contOld === contNew) return;
