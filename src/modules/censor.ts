...
export function getApplicableConfigs(member: discord.GuildMember, channel: discord.GuildChannel | undefined = undefined): Array<any> {
  ...
  if (channel instanceof discord.GuildChannel && typeof cfgMod.channels === 'object' && Object.keys(cfgMod.channels).includes(channel.id)) {
    toret.push(cfgMod.channels[channel.id]);
  }
  if (channel instanceof discord.GuildChannel && typeof channel.parentId === 'string' && channel.parentId !== '' && channel.type !== discord.Channel.Type.GUILD_CATEGORY && typeof cfgMod.categories === 'object' && Object.keys(cfgMod.categories).includes(channel.parentId)) {
...
export async function getCensorshipData(txt: string, checkInvites = false, checkUrls = false, checkZalgo = false) {
  ...
  if(checkUrls === true) {
      let urls: any = txt.match(UrlRegex);
      if(Array.isArray(urls) && urls.length > 0) {
          toret.urls = [...new Set(urls)].map((url) => new URL(url));
      }
  }
  if(checkZalgo === true) {
      let zalgo: any = txt.match(ZalgoRegex);
...
export async function getDataFromConfig(txt: string, thisCfg: any, checkWords = false, checkTokens = false, checkCaps =false, checkChars = false) {
    let toRet = {words: checkWords === true ? [] : undefined,
    ...
    if(checkWords === true) {
        ...
        found = splitting.filter((txt) => blocks.includes(txt));
        if(Array.isArray(found) && found.length > 0) {
            toRet.words = [...new Set(found)];
        }
    }
    if(checkTokens === true) {
        let blocks: any = txt.match(new RegExp(thisCfg.tokens.filter((w) => typeof w === 'string' && w.length > 0).join('|'), 'gi'));
    ...
    if(checkCaps === true) {
        ...
        if(typeof thisCfg.caps.percentage === 'number') {
            ...
            if(thisPct > thisCfg.caps.percentage) toRet.caps.push(`${thisPct}% / ${thisCfg.caps.percentage}%`);
        }
        if(typeof thisCfg.caps.followed === 'number') {
            let inARow = 0;
            for(var i = 0; i <matches.length; i+=1) {
                const thisc = matches[i];
                if(thisc.toUpperCase() === matches[i]) {inARow+=1;}else{inARow =0}
...
export function checkCensors(data: any, thisCfg: any): CensorCheck {
  ...
  const invites: Array<string | discord.Invite> = data.invites;
  const urls: Array<URL> = data.urls;
  const zalgo: Array<string> = data.zalgo;
  const words:Array<string>  = data.words;
  const tokens: Array<string>  = data.tokens;
  const caps: Array<string>  = data.caps;
  const chars: Array<string>  = data.chars;
      ...
      for(var i = 0; i < urls.length; i+=1) {
          ...
          if(Array.isArray(allowed) && !allowed.includes(host)) {
              if(allowSubdomains=== true && host.includes('.') && host.split('.').length > 2) {
                  ...
                  if(allowed.includes(actualDomain) && !denied.includes(actualDomain)) continue;
            }
  ...
  if(typeof thisCfg.chars === 'object' && Array.isArray(chars) && chars.length > 0) {
    return new CensorCheck(true, CensorType.CAPS, 'Illegal characters', chars.join(', '), _stop);
  }
