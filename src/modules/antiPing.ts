...
async function isIllegalMention(
  ...
  if (author.user.bot || target.user.bot) return false;
  if (author.user.id === target.user.id) return false;
  if (await isBypass(author)) return false;
  if (await isMuted(author)) return false; // in case mute fails, lol
  let itarget = await isTarget(target);
  if (!itarget) return false;
  let atarget = await isTarget(author);
  if (itarget && atarget) return false;
  if (await isStaff(author)) return false;
  return true;
}
...
async function isMuted(member: discord.GuildMember) {
  ...
  if (roles.some((e) => e.id === cfgMod.muteRole)) isMuted = true;
  return isMuted;
}
...
async function isStaff(member: discord.GuildMember) {
  ...
  roles.forEach(function(role) {
    if (cfgMod.staffRoles.indexOf(role.id) > -1) hasStaff = true;
  });
  return hasStaff;
}
...
async function isTarget(member: discord.GuildMember) {
  ...
  const cfgMod = config.getGuildConfig(member.guildId);
  if (cfgMod.targets.users.blacklist.indexOf(userId) > -1) return false;
  if (cfgMod.targets.users.whitelist.indexOf(userId) > -1) return true;
  let roles = (await utils.getUserRoles(member)) as Array<discord.Role>;
  let hasWhitelist = false;
  let hasBlacklist = false;
  roles.forEach(function(role) {
    if (cfgMod.targets.roles.whitelist.indexOf(role.id) > -1) hasWhitelist = true;
    if (cfgMod.targets.roles.blacklist.indexOf(role.id) > -1) hasBlacklist = true;
  });
  if (hasWhitelist && !hasBlacklist) return true;
  return false;
}
...
async function isBypass(member: discord.GuildMember) {
  ...
  let userId = member.user.id;
  const cfgMod = config.getGuildConfig(member.guildId);
  if (cfgMod.bypass.users.indexOf(userId) > -1) return true;
  if (await isIgnore(userId)) return true;
  let roles = await utils.getUserRoles(member);
  let is = false;
  roles.forEach(function(role) {
    if (cfgMod.bypass.roles.indexOf(role.id) > -1) is = true;
  });
  return is;
}
...
async function log(type: string, usr: discord.User | undefined = undefined, actor: discord.User | undefined = undefined, extras: Map<string,string> | undefined = new Map()) {
    if(usr instanceof discord.User) extras.set('_USERTAG_', logUtils.getUserTag(usr));
    if(actor instanceof discord.User) extras.set('_ACTORTAG_', logUtils.getUserTag(usr));
...
async function messageDeleted(messageId: string) {
  let data:any = await kv.get(kvDataKey);
  let changed = false;
  for (var userId in data) {
    for (var mId in data[userId]) {
      let obj = data[userId][mId]['BotReplyMsg'];
      if (obj.id === messageId) {
  ...
  if (changed) await kv.put(kvDataKey, data);
}
...
async function wipeAllUserMessages(userId: string, allMessages = false) {
  ...
  //let guild;
  for (var mid in data[userId]) {
    for (var msgkey in data[userId][mid]) {
      let obj = data[userId][mid][msgkey];
      //if (typeof guild !== 'object')
      //guild = await discord.getGuild(obj.guildId);
      if (msgkey === 'OriginalMsg' && !allMessages) continue;
      let chan = (await discord.getChannel(
        obj.channelId
      )) as discord.GuildTextChannel;
      if (typeof chan !== 'object' || chan === null) continue;
      let msg = await chan.getMessage(obj.id);
...
export async function OnMessageCreate(
  ...
  if (message.webhookId !== null || message.type !== 0) return;
  if (message.mentions.length <= 0 || message.mentions.length > 10) return;
  if (!(message instanceof discord.GuildMemberMessage)) return;
  let channel = (await message.getChannel()) as discord.GuildTextChannel;
  let author = message.author;
  let guild = await message.getGuild();
  if (guild === null) return;
  let authorMember = message.member;
  if (!(authorMember instanceof discord.GuildMember)) return;

  if (await isBypass(authorMember)) return;
  let illegalMentions = [];
  await Promise.all(
    message.mentions.map(async function(mention) {
      let ghettoMember: any = mention.member;
      let usr = mention;
      delete usr['member'];
  ...
  let msgtorep = `${message.author.toMention()} ${msg}\n${muteText}\n${
    cfgMod.actualCaughtMessage
  }`;
  let msgReply = await message.reply(msgtorep);
  ...
  for (var i = 0; i < 2; i++) {
    ...
    if (i === 0) objtarg = msgReply;
    for (var key in objtarg) {
      if (clearKeys.indexOf(key) > -1) continue;
  ...
  let pingCount = message.mentions.length;
  let pingText = '' + pingCount + ' users';
  if (pingCount < 10) {
    pingText = '';
    for (var key in message.mentions) {
      let obj = message.mentions[key];
      if (pingText !== '') pingText += ', ';
      pingText += '<@!' + obj.id + '>';
    }
  }
  if(isMute) {
  ...
  await kv.put(kvDataKey, data);
  for (var key in cfgMod.emojiActions) {
...
export async function OnMessageReactionAdd(args) {
  ...
  // Check staff adds
  if (!(await isStaff(args.member))) return;
  ...
  let thisData;
  for (var userId in data) {
    if (typeof thisData === 'object') break;
    for (var mId in data[userId]) {
  ...
  if (typeof thisData !== 'object') return;
  let botMsg = thisData['BotReplyMsg'];
  let userMsg = thisData['OriginalMsg'];
  let channel = (await discord.getChannel(
    channelId
  )) as discord.GuildTextChannel;
  const cfgMod = config.getGuildConfig(member.guildId);
  let emojiAction = cfgMod.emojiActions[emoji.name];
  if (typeof emojiAction !== 'string') return;
  ...
  async function tryGetGuildMember(guild: discord.Guild, id: string) {
    ...
    if (typeof membr !== 'object') return false;
    return membr;
  }
  ...
  let mutes = (await kv.get(kvMutesKey)) as Array<string>;
  if (!Array.isArray(mutes)) mutes = [];
  let isMuted = mutes.includes(userMsg.authorId);
  ...
  if (emojiAction === 'IgnoreOnce') {
    wipeAll = false;
    if (isMuted) {
      let membr = await tryGetGuildMember(guild, userMsg.authorId);
      if (membr === false) {
    ...
    if (membr === false) {
      ...
      try {
        ...
        if (!Array.isArray(kickDebugs)) kickDebugs = [];
...
export async function OnGuildMemberRemove(dataMember: discord.Event.IGuildMemberRemove) {
  ...
  let guild = await discord.getGuild(dataMember.guildId);
  if (typeof data[user.id] !== 'object' || guild === null) return;
  const cfgMod = config.getGuildConfig(dataMember.guildId);
  let isBan =
    Object.keys(data[user.id]).length >= cfgMod.pingsForAutoMute ||
    cfgMod.banOnLeave;
  if (!isBan) {
...
export async function OnGuildMemberUpdate(member: discord.GuildMember) {
  // Check mute remove
  let roleMuted = await isMuted(member);
  let mutesVal = await kv.get(kvMutesKey);
  if (!Array.isArray(mutesVal)) mutesVal = [];
  let valMuted = mutesVal.includes(member.user.id);
